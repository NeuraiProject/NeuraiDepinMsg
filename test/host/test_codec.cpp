 ...
 * (test/fixtures/vectors.txt = Neurai/contrib/depin/vectors.txt, §13 of
...
int main(int argc, char ** argv) {
    ...
    if (argc > 2 && strcmp(argv[2], "--dump") == 0) {
        /* cross-check mode: emit an envelope and a signed message for the
         * holder, built with the real RNG, for test/host/crosscheck.py */
        PrivateKey holder; loadPrivateKey(V["holder_wif"], holder);
        uint8_t sec[33]; { PublicKey p = holder.publicKey(); p.compressed = true; p.sec(sec, 33); }
        vector<vector<uint8_t> > rcpt(1, vector<uint8_t>(sec, sec + 33));
        const char content[] = "Hello from the C++ codec";
        DepinMessage m;
        if (messageBuild("&TEST/SEC", V["holder_address"], 1787377444, DEPIN_TYPE_GROUP,
                         (const uint8_t *)content, sizeof(content) - 1, rcpt, holder, m) != Err::Ok) return 3;
        vector<uint8_t> wire; messageSerialize(m, wire);
        printf("content=%s\n", content);
        printf("hash=%s\n", m.hash().c_str());
        printf("payload=%s\n", hexEncode(m.payload.data(), m.payload.size()).c_str());
        printf("signature=%s\n", hexEncode(m.signature.data(), m.signature.size()).c_str());
        printf("wire=%s\n", hexEncode(wire.data(), wire.size()).c_str());
        return 0;
    }
